...
```{r}
df %>%
  ggplot(aes(x=SL.EMP.WORK.ZS, y=SL.AGR.EMPL.ZS)) +
    geom_point() + 
  xlim(0,100) +
    theme_minimal()
```
...
```{r}
lm <- lm(SL.AGR.EMPL.ZS ~ SL.EMP.WORK.ZS, data=df)
```
...
```{r}
resid_plot <- tibble(
  resid_agr = residuals(lm),
  predict_agr = predict(lm, df),
  agr_orig = df$SL.AGR.EMPL.ZS)

resid_plot %>%
  ggplot(aes(x=predict_agr, y=resid_agr)) +
    geom_point() +
    xlim(-10,100) +
    theme_minimal()
