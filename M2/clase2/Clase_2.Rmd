...
Como dice Stephen Stigler -gran historiador de la disciplina-, el MCO es "el automóvil de la estadística moderna". En efecto, este método es absolutamente central en la práctica profesional y científica en las ciencias sociales. Vamos 
...
- En muchas aplicaciones puede ser útil penalizar más fuertemente los errores grandes que los pequeños. Al elevar al cuadrado los residuos, un error dos veces mayor recibe cuatro veces más peso en la función que estamos minimizando. Por ejemplo, estar equivocado en 4 suele ser más del doble de malo que estar equivocado en 2. Elevar al cuadrado los residuos da cuenta de esta discrepancia.
...
Para los datos de la población de Botswana podemos escribir ahora la ecuacion de regresión:

$$\hat{wei} =  -52.3162 + 0.6294 \times height$$

La idea, entonces, es poder "predecir" el peso utilizando la altura. Los parámetros de la recta ($\beta_{0}, \beta_{1}$) son estimados mediante los datos ($wei, height$). En la práctica, esta estimación se hace usando una computadora/software estadístico. Conocer las fórmulas permiten comprender de dónde provienen los coeficientes y permiten calcularlos manualmente, de la misma manera que otras estimaciones, como la media de una muestra.
...
> <small>**Para pensar con tranquilidad...**
¿Qué queremos decir con que son datos "observacionales"? ¿Y por qué no podemos inferir causalidad de este tipo de datos? ¿Qué tipo de datos necesitaríamos para poder inferir causalidad? </small>


El intercepto estimado es negativo. En este caso, no tiene demasiado valor de interpretación, en tanto, no podemos afirmar que existe un peso negativo (porque implicaría el peso esperado para una persona con 0 cm de altura: un valor que no pertenece al rango de valores posibles y observados de altura). Pero existen situaciones en las que es posible interpretar este parámetro como el promedio de la variable dependiente si la independiente fuese cero..


> <small> **Interpretación de parámetros estimados por mínimos cuadrados.**
La pendiente describe la diferencia estimada en el resultado promedio pronosticado de $y$ si la variable predictora $X$  pasó a ser una unidad más grande. El intercepto describe el resultado promedio de $y$ si $X=0$ y el modelo lineal es válido hasta el cero. En muchas situaciones (como en este caso) la variable $X$ no puede tener valor cero.</small>


Recordemos que una forma alternativa de calcular los valores de la intersección y la pendiente de una línea de mínimos cuadrados es haciendo cuentas a mano mediante fórmulas. Si bien los estadísticos y los científicos de datos no suelen utilizar los cálculos manuales, es útil pensar de esta forma una primera vez sobre la línea de mínimos cuadrados y el modelado en general. 
...
> <small> Si les interesa, [aquí](https://statproofbook.github.io/P/slr-ols.html) dejamos una demostración de lo que acabamos de mostrar.</small>

Vamos a tratar de calcula "a mano" los parámetros de la recta de regresión con los datos que estamos trabajando. Primero obtenemos media, varianza y covarianza de cada variable:
...
Ahora, podemos reemplazar los valores en las ecuaciones anteriores:
...
aproximadmante el 57% de la variación de la variable dependiente (`wei`) al usar información sobre la `height` y modelando su relación de forma lineal. El 43% restante corresponde a la variabilidad que no es capturada por este modelo lineal. Resulta que $R^2$ corresponde exactamente al valor al cuadrado de la correlación:
...
La pendiente ($\beta_{1}$) puede ser interpreatada de la siguiente manera: si el caso es varón entonces, debemos sumarle al promedio del peso 6.778 kg. Es decir, que los varones son en promedio 6.77kg. más pesados que quienes no son varones.

---
  
**Interpretación de las estimaciones del modelo para predictores categóricos**

El intercepto estimado es el valor de la variable de resultado para la primera categoría (la categoría de referencia es la que se encuentra codificada con el valor de indicador 0). La pendiente estimada es el cambio promedio en la variable de resultado entre las dos categorías.
...
**Tipos de outliers (valores atípicos)**
...
Un punto (o un grupo de puntos) que sobresale del resto de los datos se denomina valor atípico. Los outliers que se alejan horizontalmente del centro de la nube de puntos se denominan puntos de apalancamiento. Los outliers que influyen en la pendiente de la línea se denominan puntos influyentes. Un punto influyente es aquel cuya presencia tiene un efecto importante sobre los resultados del modelo. Un buen ejercicio de práctica, por ejemplo, puede ser evaluar cuánto cambian los coeficientes estimados al ajustar el modelo: incluyendo y excluyendo esa observación.

---

Es tentador proceder a eliminar los valores atípicos sin demasiada más reflexión que mirar un boxplot. En general, no es una buena idea hacerlo sin una buena razón. Los modelos que ignoran casos excepcionales (e interesantes) a menudo funcionan mal. Por ejemplo, si una empresa financiera ignorara las oscilaciones más grandes del mercado (los "valores atípicos") pronto quebraría debido a inversiones mal pensadas.  
Un valor atípico no es necesariamente un error. Puede representar un caso válido e informativamente importante por lo cual, antes de decidir si lo vamos a eliminar, deberíamos investigar por qué se encuentra alejado del resto de los datos.
