...
La variable de resultado para un GLM (acrónimo de "**G**eneralized **L**inear **M**odel") se denota por $Y_{i}$, donde el índice $i$ se utiliza para representar la _i-ésima_ observación. En la ENES, $Y_{i}$ se utilizará para representar si la persona $i$ tiene percepción de clase obrera ($Y_{i} = 1$) o no ($Y_{i} = 0$).
...
\begin{equation}
  Y=\left\{
  \begin{array}{@{}ll@{}}
    0, & \text{si percep_obrera = Sí} \\
    1, & \text{si percep_obrera = No}
  \end{array}\right.
\end{equation} 
$$
---
...
Por ejemplo, ¿cómo se distribuyen los casos según los ingresos individuales?
...
Necesitamos, entonces, alguna forma para que el modelo de regresión no haga predicciones fuera del rango 0-1, que es el rango válido que puede asumir una probabilidad:

$$transformacion(p_{i}) = \beta_{0} + \beta_{1}X_{1,i} + \beta_{2}X_{2,i} + ... + + \beta_{k}X_{k,i}$$
Por ejemplo, queremos una transformación que haga que el rango de posibilidades del lado izquierdo de la ecuación sea igual al rango de posibilidades del lado derecho. Si no hubiera transformación para esta ecuación, el lado izquierdo solo podría tomar valores entre 0 y 1, pero el lado derecho podría tomar valores fuera de este rango. 

Una transformación común para evitar que los valores estimados caigan fuera del rango 0-1 es calcular el *logit* que puede ser escrito de la siguiente manera:
...
Si bien la elección precisa de una función logit no es intuitiva, se basa en la teoría que sustenta los modelos lineales generalizados, que está más allá del alcance de este curso. Afortunadamente, una vez que ajustamos un modelo usando software, empezaremos a sentir que estamos de vuelta en el contexto de la regresión múltiple, incluso si la interpretación de los coeficientes es un poco más compleja.

Si aplicamos la transformación logit, vemos que modelando el logit (no la probabilidad) lo estamos haciendo como una función lineal de $X_{k}$:

$$
log_e\left(\frac{p_{i}}{1-p_{i}}\right) =  \beta_{0} + \beta_{1}X_{1,i}
$$
...
$$
log_e\left(\frac{p_{i}}{1-p_{i}}\right) =  \beta_{0} + \beta_{1}X_{1,i}\\
\frac{p_{i}}{1-p_{i}} = e^{  \beta_{0} + \beta_{1}X_{1,i}} \\
p_{i} = (1-p_{i})  e^{  \beta_{0} + \beta_{1}X_{1,i}} \\
p_{i} = e^{  \beta_{0} + \beta_{1}X_{1,i}} - p_{i} \times e^{  \beta_{0} + \beta_{1}X_{1,i}} \\
p_{i} +  p_{i} \times e^{  \beta_{0} + \beta_{1}X_{1,i}} = e^{  \beta_{0} + \beta_{1}X_{1,i}} \\
p_{i} (1 + e^{  \beta_{0} + \beta_{1}X_{1,i}}) = e^{  \beta_{0} + \beta_{1}X_{1,i}} \\
p_{i} = \frac{e^{  \beta_{0} + \beta_{1}X_{1,i}}}{ 1 + e^{  \beta_{0} + \beta_{1}X_{1,i}}}
$$
Veamos ahora, qué pasaría si ajustamos una regresión con la transformación logit aplicada, es decir, una regresión logística:
...
**Actividad 1**
Estimen un modelo de regresión logística que intente predecir la autopercepción obrera según clase social:
...
Ajustemos el modelo de regresión logística con 5 predictores. Pensemos brevemente las fórmulas: en el ejemplo, vamos a usar unas 5 variables predictoras, por lo cual $k=5$. Al igual que con la mayoría de los problemas de datos aplicados, sustituimos los parámetros ($\beta_{k}$) por las estimaciones puntuales ($\hat{\beta_{k}}$). Y escribimos nuevamente la ecuación que relaciona a los predictores con $Y_{i}$ usando la transformación logit sobre $p_{i}$:

$$log_e\left(\frac{p_{i}}{1-p_{i}}\right) =  \beta_{0} + \beta_{1}X_{1,i} + \beta_{2}X_{2,i} + ... + + \beta_{k}X_{k,i}$$

Y, cuando querramos volver a tener la información en la escala de probabilidad (no de logit), el procedimiento que vimos para el caso de un solo predictor es directamente generalizble:
...
Al igual que la regresión múltiple, el resultado se puede presentar en una tabla de resumen, que se muestra en la tabla siguiente.
