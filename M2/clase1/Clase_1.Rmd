...
Veamos el punto de máximo residuo: es el que tiene `height=``df_mayores[which.max(df_mayores$resid),"height"]`
...
```{r}
df_mayores <- df_mayores %>%
        mutate(height_predicted = predict(lm_1, df_mayores)) %>%
        mutate(height_resid = height - height_predicted)
