...
ACA PODEMOS HACER INGRESO PER CAPITA DEL HOGAR EDAD MEDIANA Y TAMAÑO PROMEDIO DEL HOGAR SEGUN REGION
...
df %>%
        ...
        ungroup() %>%
        group_by(REGION) %>%
        summarise(itf_medio = mean(ITF),
                  edad_media = mean(CH06),
                  cant_miembros = mean(cant_miembros)) %>%
  gt::gt() %>%
  gt::fmt_number(
                columns=-c(REGION),
                decimals = 2)
...
Tenemos datos de 3142 condados en los Estados Unidos. Nos faltan los datos de población de 2017 de 3 de ellos, y de los 3139 condados restantes, en 1541 la población aumentó de 2010 a 2017 y en los 1598 restantes la población disminuyó. La tabla 4.6 muestra una muestra de 5 observaciones de cada grupo.
