...
```{r}
df <- get_microdata(year=2020, trimester=2, type='individual')
...
```
- Con `mutate_at` podemos editar varias variables al mismo tiempo:
  + le pasamos el parámetro `vars()` donde indicamos el conjunto de columnas que queremos modificar
  + para aplicar una función hay que introducirla con el símbolo `~`
  + usamos el punto `.` para referirnos a la selección de variables anterior
```{r}
df <- df %>% mutate_at(vars(CH03, sexo, CH07:CAT_INAC), ~as.character(.))
...
medidas_resumen <- df %>%
  ...
  summarise(media_edad = mean(CH06, na.rm = TRUE),
            mediana_edad = median(CH06, na.rm = TRUE)) %>% 
  select(sexo, 
         media_edad, mediana_edad) 

pivot_longer_resumen <- medidas_resumen%>% 
  pivot_longer(cols = c(media_edad, mediana_edad), 
               names_to = "variable", 
               values_to = "valor")
```
...
```{r}
pivot_longer_resumen%>% pivot_wider(id_cols= sexo, 
                                names_from=variable,
                                values_from = valor)
```
...
Supongamos que queremos juntar la base que tenemos con la variable del monto del ingreso total familiar, que está en el registro de hogares. La columna **CODSU** nos da un identificador único de cada registro. De manera que podemos hacer un seguimiento de los individuos en el hogar a través de esa columna
```{r}
df_2 <- get_microdata(year=2020, trimester=2, type='hogar')  %>% select(CODUSU, ITF) # Selecciono únicamente las variables de ID y el ingreso total familiar

df_2 <- df_2%>% organize_labels(type='hogar')

df_merged <- df %>% left_join(df_2, by = "CODUSU")
